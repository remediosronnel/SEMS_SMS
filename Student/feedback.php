...
$sql1 = "SELECT * FROM rso_attentable WHERE rso_attentable.studentID = '$studentID'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_GET['studentID'])) {
        ...
        $sql_select = "SELECT * FROM rso_attentable WHERE studentID = '$studentID'";
        $result_select = mysqli_query($conn, $sql_select);

        if ($result_select && mysqli_num_rows($result_select) > 0) {
            // If record exists, proceed with the update
            $sql_update = "UPDATE rso_attentable SET satRate = '$satisfaction', sugComment = '$suggestion' WHERE studentID = '$studentID' AND eventID = '$eventID'";
